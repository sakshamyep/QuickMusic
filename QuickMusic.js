...
class QuickMusic {
  ...
  handlePlayerStart = async (player, track) => {
    ...
    await this.taskQueue.enqueue(async () => {
      if (player.queue.length < 1 && isAutoplayEnabled) {
        await this.addRelatedTrack(player).catch((error) => {});
      }
      await this.sendNowPlayingEmbed(player).catch((error) => {});
    });
  };

  handlePlayerEnd = async (player) => {
  if (!player || player.destroyed || !player.guildId) {
    return null;
  }
  ...
  await this.taskQueue.enqueue(async () => {
    ...
    } else if (isAutoplayEnabled && !this.skipInProgress && player.queue.length < 1) {
      const added = await this.addRelatedTrack(player).catch(() => false);
      if (added && player.queue.current && !player.playing) {
        await player.play().catch(() => {});
        await this.sendNowPlayingEmbed(player).catch(() => {});
      }
    }
  });
}};
...
quickMusic.kazagumo.shoukaku.on('disconnect', (name, reason) => {
  console.warn(`🔴 Node Disconnected: ${name}, Reason: ${reason}`);
  setTimeout(async () => {
    try {
...
quickMusic.kazagumo.shoukaku.on('error', (name, error) =>
